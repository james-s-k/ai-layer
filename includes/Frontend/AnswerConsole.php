<?php
/**
 * Shared answer engine console: renders the query form, result panel, and JS.
 * Used by the admin test page and the [wpail_answer_console] shortcode.
 *
 * @package WPAIL\Frontend
 */

declare(strict_types=1);

namespace WPAIL\Frontend;
...
class AnswerConsole {
	...
	/**
	 * Render the card — the self-contained query form + result panel + JS.
	 *
	 * @param bool $send_nonce Pass true in admin contexts to include a WP REST nonce.
	 */
	public static function render_card( bool $send_nonce = false ): void {
		$services  = ( new ServiceRepository() )->get_all();
		$locations = ( new LocationRepository() )->get_all();
		$rest_url  = rest_url( 'ai-layer/v1/answers' );
		$nonce     = $send_nonce ? wp_create_nonce( 'wp_rest' ) : '';
		?>
		<div class="wpail-card wpail-answer-console" style="max-width: 860px;">
